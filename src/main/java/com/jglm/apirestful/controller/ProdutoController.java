...
import com.jglm.apirestful.model.ProdutoDTO;
import com.jglm.apirestful.model.ResultadoPaginado;
import com.jglm.apirestful.service.ProdutoService;
...
public class ProdutoController {
    ...
    // recuperarProdutoPorId(@PathVariable("idProduto") Long id) {
    ...
    @GetMapping("{idProduto}")
    public ResponseEntity<?> recuperarProdutoPorId(
            @PathVariable("idProduto") Long id,
            @RequestParam(name = "incluirCategoria", defaultValue = "true") boolean incluirCategoria) {
        if (incluirCategoria) {
    ...
    // Implementação que retorna um Produto ou um String (mensagem de errro) sem
    // utilizar
    // um GlobalExceptionHandler
    // // Get para http://localhost:8080/produtos/1
    // @GetMapping("{idProduto}")
    // public ResponseEntity<?> recuperarProdutoPorId(@PathVariable("idProduto")
    // Long id) {
    // try {
    // Produto produto = produtoService.recuperarProdutoPorId(id);
